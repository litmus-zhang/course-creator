...
class ResearchReport(BaseModel):
    ...
    concepts: list[str] = Field(description="Core concepts and definitions related to the topic.")
    recommended_modules: list[str] = Field(description="Syllabus modules or structure suggested.")
    learning_outcomes: list[str] = Field(description="Key learning outcomes for the course.")
    detailed_summary: str = Field(description="Detailed research summary from web search.")

class CritiqueResult(BaseModel):
    verdict: Literal["APPROVED", "REJECTED"] = Field(description="APPROVED if the research is complete, otherwise REJECTED.")
    feedback: str = Field(description="Detailed critique and suggestions.")
    missing_aspects: list[str] = Field(description="List of specific things missing from the research report.")
...
class LessonOutline(BaseModel):
    ...
    summary: str = Field(description="Summary outline of what is taught in this lesson.")
...
class ModuleOutline(BaseModel):
    ...
    lessons: list[LessonOutline] = Field(description="List of detailed lessons (at least 3 lessons per module).")
...
def check_verdict(ctx: Context, node_input: CritiqueResult) -> Event:
    ...
    if node_input.verdict == "APPROVED":
        ...
        return Event(output=node_input, route="continue", state={"loop_count": loop_count})
